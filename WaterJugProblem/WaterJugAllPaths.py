...
def checkState(allPahts, state):
    temporyQueue = allPahts
    print('temp queue 1', temporyQueue)
    
    while (not (not temporyQueue)):
        if temporyQueue[0] == state:
            print('while false:', temporyQueue[0],state )
            return False
        else:
            temporyQueue.pop()
            
        print('temp queue', temporyQueue)
    return True
...
def solution():
    ...
    while(not (not queue)):
        ...
        if (checkState(path, current_node)):
